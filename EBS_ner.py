# -*- coding: utf-8 -*-
"""
Created on Fri Feb 22 16:30:52 2019
...
#df_fin['Named_Entity'][1]
gram = []
for i in range(0,len(df_fin['Named_Entity'])):
    print(i)
    if(len(df_fin['Named_Entity'][i].split()) == 1) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][1].split(), 1, 1)))
    if(len(df_fin['Named_Entity'][i].split()) == 2) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 2)))
    if(len(df_fin['Named_Entity'][i].split()) == 3) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 3)))
    if(len(df_fin['Named_Entity'][i].split()) == 4) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 4)))
    if(len(df_fin['Named_Entity'][i].split()) == 5) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 5)))
    if(len(df_fin['Named_Entity'][i].split()) == 6) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 6)))
    if(len(df_fin['Named_Entity'][i].split()) == 7) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 7)))
    if(len(df_fin['Named_Entity'][i].split()) == 8) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 8)))
    if(len(df_fin['Named_Entity'][i].split()) >= 9) == True:
        gram.append(list(everygrams(df_fin['Named_Entity'][i].split(), 1, 9)))
         

        
gr = []       
for i in range(0,len(df_fin['Named_Entity'][1:500])):
    print(i)
    gr.append(set(process.extract(df_fin['Named_Entity'][i], ad, limit=5)))
...
sas = pd.DataFrame(s)
sas.columns = ['NER_Grams']
sas.to_csv('C:/Users/mayur.v/Desktop/BHGE/Named_Entity_Model_gram.csv')
...
import itertools
dd = list(itertools.product(gr))
    

set(df_fin['Named_Entity'][0])&set(ad)

        
        
fuzz.token_sort_ratio("inspection inlet air sand plenum", ad[364])

        
ad = []
for i in range(0,len(df['PMHD_TA005_GRP_T_DES'])):
    print(i)
    ad.append(df['PMHD_TA005_GRP_T_DES'][i])
    
ad = [x.lower() for x in ad]

from fuzzywuzzy import fuzz
from fuzzywuzzy import process        
fuzz.token_sort_ratio("inspection inlet air sand plenum", ad[350:360])

dc = ['air sand inlet plenum inspection']
ad = pd.DataFrame(ad)
ad.columns = ['NER']

fuzz.token_sort_ratio("air sand inlet plenum inspection", "plenum sand")


wd = []
for i in range(0,len(df_fin['Named_Entity'][1:6])):
    print(i)
    for j in range(0,len(ad['NER'])):
        if(fuzz.token_set_ratio(df_fin['Named_Entity'][i], ad['NER'][j])==100)==True:
            wd.append(ad['NER'][j]) 

            
            
            
[(x,y) for x in df_fin['Named_Entity'][0] for y in    dc ]

for i, ele in enumerate(df_fin['Named_Entity'][0]):
    if(fuzz.token_set_ratio(df_fin['Named_Entity'][0], dc[0])==100)==True:
            #wd.append(ad['NER'][j])
            print(ele, dc[0])



process.extract("inspection inlet air sand plenum", sa1, limit=5)
...
n = 6
sixgrams = ngrams(sentence.split(), 1)
